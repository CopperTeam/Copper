...
using Copper.Core.ComponentModel;
            ...
            using (new WriteLockDisposable(Locker))
            {
                ...
                try
                {
                    ...
                    foreach (var dfd in GetDescriptionFilesAndDescriptors(pluginFolder))
                    {
                        ...
                        if (!pluginDescriptor.SupportedVersions.Contains(CopperVersion.CurrentVersion, StringComparer.InvariantCultureIgnoreCase))
                        {
        ...
        private static Assembly PerformFileDeploy(FileInfo plug)
        {
            if (plug.Directory.Parent == null)
                throw new InvalidOperationException("The plugin directory for the " + plug.Name +
                                                    " file exists in a folder outside of the allowed Copper folder heirarchy");
